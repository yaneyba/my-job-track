...
function generateCalendarSvg(size = 24, strokeColor = 'currentColor', bgColor = 'none', strokeWidth = 2) {
  const viewBoxSize = 24; // Keep viewBox consistent for proper rendering
...
async function generateIcons() {
  ...
  const faviconSvg = generateCalendarSvg(24, 'currentColor', 'none', 2);
  fs.writeFileSync(path.join(publicDir, 'favicon.svg'), faviconSvg);
  console.log('Generated favicon.svg');
  
  // Smaller 32x32 icon with blue stroke
  const favicon32Svg = generateCalendarSvg(32, '#3b82f6', 'none', 1.5);
